...
このとき、変換後のベクトルから高々$k$まで要素が欠落しても元のデータが復元出来る場合に$M$をMDS生成行列と呼ぶ。$k+1$以上の要素が欠落すると復元できない。
...
\end{pmatrix}
$$
という $\{ b_1, b_2, \ldots, b_n \} \subset \{ a_1, a_2, \ldots, a_{n+k} \}$ を満たす正方部分行列は正方ヴァンデルモンド行列であるから明らかに可逆である。
...
そこで$32_{\text{Byte}}/4_{\text{Byte}} = 8$分割した上で、そのそれぞれで符号化と復号化を行うことにする。
